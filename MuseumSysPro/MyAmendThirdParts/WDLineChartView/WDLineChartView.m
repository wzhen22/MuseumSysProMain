...
- (void)doWithCalculate{
    ...
    CGFloat ddWitth = self.bgView.frame.size.width/self.xValues.count;
    _everyW = ddWitth>_everyW?ddWitth:_everyW;
    // 移除多余的值，计算个数
...
- (void)drawXYLabels{
    ...
    //Y轴
    //背景view
    UIView *bgy = [[UIView alloc] initWithFrame:CGRectMake((CGRectGetWidth(self.bgView.frame) -_rightMargin+2),0, _rightMargin-2, CGRectGetHeight(self.bgView.frame))];
    bgy.backgroundColor = [UIColor whiteColor];
    [self addSubview:bgy];
    
    for(int i = 0; i <= _yCount; i ++)
    {
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake((CGRectGetWidth(self.bgView.frame) -_rightMargin+2),(_topMargin-_everyH * 0.5)+i*_everyH, _rightMargin-2, _everyH)];
    ...
    _yNameLabel = [[UILabel alloc] initWithFrame:CGRectMake((CGRectGetWidth(self.bgView.frame) -85),0, 80, 20)];
    _yNameLabel.backgroundColor = [UIColor clearColor];
    _yNameLabel.textColor = self.yColor;
    _yNameLabel.font = [UIFont systemFontOfSize:15];
    _yNameLabel.textAlignment = NSTextAlignmentCenter;
    _yNameLabel.text = @"(巡检次数)";
    [self addSubview:_yNameLabel];
